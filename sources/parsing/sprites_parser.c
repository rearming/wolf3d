...
int				get_all_weapons(const char **files, t_anim *weapons)
{
	...
	weapons[0] = (t_anim){GUN, 0, (t_point) {WIN_WIDTH / 3 + 100,
			WIN_HEIGHT - 256 * 2, 0, 0}, 6, 0.013, 2};
	weapons[1] = (t_anim){KNIFE, 0,
			(t_point) {WIN_WIDTH / 3, WIN_HEIGHT - 192 * 4, 0, 0}, 4, 0.011, 4};
	weapons[2] = (t_anim){DAKKA, 0,
			(t_point) {WIN_WIDTH / 3, WIN_HEIGHT - 192 * 4, 0, 0}, 11, 0.02, 4};
	weapons[3] = (t_anim){PICKAXE, 0,
			(t_point) {WIN_WIDTH / 3, WIN_HEIGHT - 200 * 4, 0, 0}, 1, 0.01, 4};
...
t_sprite		*get_minecraft_art(const char *filename)
{
	...
	pictures[0] = get_sprite(img, 32, (t_point){0, 128, 0, 0});
	pictures[1] = get_sprite(img, 32, (t_point){32, 128, 0, 0});
	pictures[2] = get_sprite(img, 32, (t_point){64, 128, 0, 0});
	pictures[3] = get_sprite(img, 32, (t_point){96, 128, 0, 0});
	pictures[4] = get_sprite(img, 32, (t_point){128, 128, 0, 0});
	pictures[5] = get_sprite(img, 64, (t_point){0, 192, 0, 0});
	pictures[6] = get_sprite(img, 64, (t_point){64, 192, 0, 0});
	pictures[7] = get_sprite(img, 64, (t_point){128, 192, 0, 0});
	free(img.data);
	return (pictures);
}
